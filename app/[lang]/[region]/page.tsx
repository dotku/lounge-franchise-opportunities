...
export default function Home({ params }: PageProps) {
  ...
  return (
    <main className="min-h-screen">
      ...
      {/* China Market Special Information */}
      {region === "china" && regionConfig.businessModel && regionConfig.minimumPurchase && (
        <section className="py-12 px-4 bg-gradient-to-r from-red-50 via-amber-50 to-yellow-50">
          <div className="max-w-5xl mx-auto">
            <div className="bg-white rounded-2xl shadow-2xl p-8 md:p-12 border-4 border-red-500">
              <div className="flex items-center mb-8">
                <span className="text-6xl mr-4">🇨🇳</span>
                <div>
                  <h2 className="text-4xl font-bold bg-gradient-to-r from-red-600 to-amber-600 bg-clip-text text-transparent">
                    {lang === "zh" ? "中国市场合作模式" : "China Market Partnership Model"}
                  </h2>
                  <p className="text-xl text-gray-600 mt-2">
                    {lang === "zh" ? "经销商战略合作" : "Strategic Distributor Partnership"}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div className="bg-gradient-to-br from-red-50 to-pink-50 p-6 rounded-xl border-2 border-red-200">
                  <div className="flex items-center mb-3">
                    ...
                    <div className="text-sm font-semibold text-gray-600">
                      {lang === "zh" ? "合作模式" : "Business Model"}
                    </div>
                  </div>
                  <div className="text-2xl md:text-3xl font-bold text-red-700">
                    {regionConfig.businessModel[lang]}
                  </div>
                  <p className="text-sm text-gray-600 mt-2">
                    {lang === "zh"
                      ? "与本地合作伙伴共同开拓市场"
                      : "Develop market with local partners"}
                  </p>
                </div>
                ...
                <div className="bg-gradient-to-br from-amber-50 to-yellow-50 p-6 rounded-xl border-2 border-amber-300">
                  <div className="flex items-center mb-3">
                    <span className="text-3xl mr-3">💰</span>
                    <div className="text-sm font-semibold text-gray-600">
                      {lang === "zh" ? "建议采购量" : "Recommended Purchase"}
                    </div>
                  </div>
                  <div className="text-2xl md:text-3xl font-bold text-amber-700">
                    ${regionConfig.minimumPurchase.amount.toLocaleString()}
                  </div>
                  <p className="text-sm text-gray-600 mt-2">
                    {regionConfig.minimumPurchase.note?.[lang]}
                  </p>
                </div>
              </div>

              <div className="bg-gradient-to-r from-red-50 to-amber-50 p-6 rounded-xl border-l-4 border-red-500 mb-6">
                <div className="flex items-start">
                  <span className="text-3xl mr-4 flex-shrink-0">💡</span>
                  <div>
                    <h3 className="text-lg font-bold text-gray-800 mb-2">
                      {lang === "zh" ? "为什么选择经销商模式？" : "Why Distributor Model?"}
                    </h3>
                    <p className="text-gray-700">
                      {lang === "zh"
                        ? "考虑到中国市场的特殊性和规模，我们采用经销商合作模式能够更好地本地化运营，快速响应市场需求。经销商将获得独家区域代理权和全面的品牌支持。"
                        : "Given the uniqueness and scale of the Chinese market, our distributor partnership model allows for better localized operations and rapid market response. Distributors receive exclusive regional rights and comprehensive brand support."}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white p-4 rounded-lg border-2 border-gray-200 text-center">
                  ...
                  <div className="font-semibold text-gray-800">
                    {lang === "zh" ? "产品供应保障" : "Product Supply"}
                  </div>
                </div>
                <div className="bg-white p-4 rounded-lg border-2 border-gray-200 text-center">
                  <div className="text-2xl mb-2">🎯</div>
                  <div className="font-semibold text-gray-800">
                    {lang === "zh" ? "区域独家代理" : "Exclusive Territory"}
                  </div>
                </div>
                <div className="bg-white p-4 rounded-lg border-2 border-gray-200 text-center">
                  <div className="text-2xl mb-2">🚀</div>
                  <div className="font-semibold text-gray-800">
                    {lang === "zh" ? "品牌市场支持" : "Brand Marketing Support"}
                  </div>
                </div>
              </div>
            </div>
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    {[
                      ...
                      { icon: "📧", label: "emailLabel", value: regionConfig.email, link: `mailto:${regionConfig.email}` },
                      { icon: "📱", label: "phoneLabel", value: regionConfig.phone, link: `tel:${regionConfig.phone.replace(/\s+/g, '')}` },
                    ].map((contact, idx) => (
        ...
        <div className="section-container">
          <div className="grid md:grid-cols-3 gap-8 mb-8">
            ...
            <div>
              ...
              <ul className="space-y-2 text-gray-400">
                <li>Xianji Li</li>
                <li>{regionConfig.email}</li>
                <li>{regionConfig.phone}</li>
                <li>{regionConfig.city[lang]}, {regionConfig.country[lang]}</li>
              </ul>
            </div>
          </div>
